...
class RTONDecoder():
	...
	def parse_utf8_text(self, fp):
		...
		i2 = len(string)
		if i1 != i2:
			self.warning_message("静默错误: " + fp.name + " 位置 " + str(fp.tell()) + ": 发现字符长度为" + str(i2) + " 的 Unicode 字符串，预期为 " + str(i1))
		return string
	...
	# type 85
		...
		try:
			...
			if str(k) == 'b""':
				if self.repairFiles:
					self.warning_message("静默错误: " + fp.name + " 位置 " + str(fp.tell()) + ":  文件结束")
				else:
					raise EOFError
			else:
	...
	# type 86
		...
		try:
			...
			if str(k) == 'b""':
				...
				raise TypeError("未知标签 " + k.args[0].hex())
		except (error, IndexError):
			if self.repairFiles:
				self.warning_message("静默错误: " + fp.name + " 位置 " + str(fp.tell()) + ": 文件结束")
			else:
		...
		i2 = len(items)
		if i1 != i2:
			self.warning_message("静默错误: " + fp.name + " 位置 " + str(fp.tell()) + ": 发现长度为 " + str(i1) + " 的数组，预期为 " + str(i2))
		if i2 != 0:
			if self.sortValues:
